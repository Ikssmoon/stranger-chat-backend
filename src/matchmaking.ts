...
function isCompatible(
  aFilter: UserFilter, aId: string, aBlocked: Set<string>,
  bFilter: UserFilter, bId: string, bBlocked: Set<string>,
): boolean {
...
export function enqueue(entry: QueueEntry): QueueEntry | null {
  const matchIndex = queue.findIndex(
    c => isCompatible(
      entry.filter, entry.socketId, entry.blockedIds,
      c.filter, c.socketId, c.blockedIds,
    ),
  );
